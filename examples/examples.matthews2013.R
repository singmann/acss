...
for (i in spans) {
  matthews2013[,paste0("K2_span", i)] <- sapply(local_complexity(matthews2013$string, span = i, alphabet=2), mean)
}
...
m_step <- stepAIC(m_initial, scope = as.formula(paste("~", paste(paste0("K2_span", spans), collapse = "+"))))
summary(m_step)

m_initial2 <- lm(as.formula(paste("mean ~", paste(paste0("K2_span", spans), collapse = "+"))), matthews2013)
